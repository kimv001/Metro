...
CREATE PROCEDURE [rep].[010_rep_recreate_repviews] AS BEGIN /*
    Developed by:            metro
...
SELECT tablename AS TABLE_NAME,
 ...
 WHERE bk IS NOT NULL -- Initialize loop variables
   ...
   AND TABLE_NAME = @tablename -- Generate the column hash list
    ...
    SELECT @columnhashlist = string_agg('ISNULL(LTRIM(RTRIM(CAST([' + COLUMN_NAME + '] AS VARCHAR(' + CASE
                                                                                                          WHEN COLUMN_NAME LIKE '%desc'
                                                                                                               OR COLUMN_NAME LIKE '%script' THEN '8000'
                                                                                                          ELSE '255'
                                                                                                      END + ')))),''-'')', ' + ''|'' + ')
 ...
 DECLARE @errormessage varchar(4000) = error_message() DECLARE @errorseverity INT = error_severity() DECLARE @errorstate INT = error_state() raiserror (@errormessage, @errorseverity, @errorstate) END catch END
