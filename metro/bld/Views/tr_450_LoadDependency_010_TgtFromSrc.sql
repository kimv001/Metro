...
*/  WITH base AS
 ...
 SELECT bk_target = dd.bk_child ,

               bk_source = dd.bk_parent -- make the views look like tables (remove prefix and postfix)
 ,

               bk_targetclean = concat(tgt.bk_schema, '||', tgt.bk_group, '|', tgt.shortname, '|') ,

               bk_sourceclean = CASE
                             WHEN src.bk_schema = 'DWH|csl' THEN concat(replace(src.bk_schema, 'csl', left(src.prefix, 3)), '||', tgt.bk_group, '|', tgt.shortname, '|')
         ...
         WHERE 1 = 1 --and dd.code like '%wmp%|internet%'

           AND dd.dependencytype = 'SrcToTgt' --select * from bld.vw_Dataset
...
,
 ...
 SELECT bk_target = tgt.bk ,
          ...
          JOIN bld.vw_dataset tgt
            ON tgt.bk = b.bk_targetclean --Where src.bk = tgt.bk
