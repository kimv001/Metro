...
,
       ...
       repositorystatuscode = rtrs.code -- select *

  FROM [rep].[vw_export] e

  LEFT JOIN [rep].[vw_datasetsrcfileformat] ff
    ON e.bk_fileformat = ff.bk

  LEFT JOIN bld.vw_dataset ds
    ON ds.datasetname = e.bk_dataset

   AND ds.bk_reftype_objecttype = 'OT|T|Table'

  LEFT JOIN bld.vw_schema s
    ON ds.bk_schema = s.bk

  LEFT JOIN bld.vw_reftype rt
    ON ds.bk_reftype_objecttype = rt.bk

  JOIN rep.vw_reftype rtrs
    ON rtrs.bk = e.bk_reftype_repositorystatus ;
