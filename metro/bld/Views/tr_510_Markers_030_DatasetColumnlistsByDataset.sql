...
*/ WITH base AS
        ...
        (SELECT bk = tgt.bk ,
               ...
               mta_rectype = diff.rectype --, tgt.TGT_ObjectType
 ,

               createdummies = CASE
                              WHEN tgt.createdummies = 1
                                   AND tgt.tgt_objecttype = 'Table' THEN 1
       ...
       markerbuild AS
        ...
        (SELECT src.bk_dataset ,
               ...
               marker = '<!<TGT_DDLcolumnlist>>' -- example value:
 ...
 ,
               ...
               markervalue = string_agg(convert(varchar(MAX), quotename(a.attributename) + ' ' + a.[ddl_type2] + char(10)) , char(9) + ', ') within GROUP (
                                                                                                                                                      ORDER BY cast(a.ordinalposition AS int)) --STRING_AGG(
 --		CHAR(9)+CONVERT(VARCHAR(max),
 --			QUOTENAME(A.AttributeName)+' ' +a.[DDL_Type2]
 --			+char(10)
 --		)
--	,', ' ) WITHIN GROUP (ORDER BY  cast(a.OrdinalPosition as int))
 , markerdescription = 'Start marker at posistion with 1 tab
...
' + char(9) + ', [delivered_datesk] [int] NULL'
     ...
     UNION ALL SELECT src.bk_dataset ,
               ...
               marker = '<!<TGT_ColumnList>>' -- example value:
 ...
 ,
     ...
     UNION ALL SELECT src.bk_dataset ,
               ...
               marker = '<!<SRC_ColumnList_SRC>>' -- Example Value:
 ...
 ,
     ...
     UNION ALL SELECT src.bk_dataset ,
               ...
               marker = '<!<SRC_ColumnList>>' -- Example value:
 --  [Id], [Name], [ProductCode], [Description], [QuantityScheduleType], [QuantityInstallmentPeriod], [NumberOfQuantityInstallments], [RevenueScheduleType], [RevenueInstallmentPeriod], [NumberOfRevenueInstallments], [CanUseQuantitySchedule], [CanUseRevenueSchedule], [IsActive], [CreatedDate], [CreatedById], [LastModifiedDate], [LastModifiedById], [SystemModstamp], [Family], [ExternalDataSourceId], [ExternalId], [DisplayUrl], [QuantityUnitOfMeasure], [IsDeleted], [IsArchived], [LastViewedDate], [LastReferencedDate], [StockKeepingUnit], [External_Id__c], [Product_Id__c], [SLA_hours__c]
 ,
     ...
     UNION ALL SELECT src.bk_dataset ,
               ...
               marker = '<!<SRC_BK_SRC>>' -- Example value:
 -- UPPER(CONCAT( ISNULL(CONVERT(NVARCHAR(4000),SRC.[Id]), N''),N''))
 ,

               markervalue = 'UPPER(CONCAT(' + string_agg('ISNULL(CONVERT(NVARCHAR(4000),SRC.' + convert(varchar(MAX), quotename(a.attributename)) + '), N'''')' , '+''|''+') within GROUP (
                                                                                                                                                                                                 ORDER BY cast(a.businesskey AS int)) + ',N''''))' ,
         ...
         WHERE 1 = 1
           ...
           AND cast(a.businesskey AS int) > 0 and((d.[firstdefaultdwhview] = 1 )--and d.LayerName = 'dwh')

                                            OR (left(d.prefix, 2) = 'tr')-- and d.LayerName = 'pub')
)
     ...
     UNION ALL SELECT src.bk_dataset ,
               ...
               marker = '<!<SRC_Dummies_Unknown>>' -- Example value:
 ...
 ,
               ...
               markervalue = string_agg(convert(varchar(MAX), [rep].[getdummyvaluebyattributebk](a.bk, '-2', '<Unknown>', '9999-12-31') + ' AS ' + quotename(a.attributename)) , ',') within GROUP (
                                                                                                                                                                                                         ORDER BY cast(a.ordinalposition AS int)) , markerdescription = ''
         ...
         WHERE 1 = 1
           ...
           AND cast(isnull(a.ismta, 0) AS int) = 0 --and src.[SchemaName] = 'dim'
 --and src.BK_RefType_ObjectType = 'OT|T|Table'
     ...
     UNION ALL SELECT src.bk_dataset ,
               ...
               marker = '<!<SRC_Dummies_Empty>>' -- Example value:
 ...
 ,
               ...
               markervalue = string_agg(convert(varchar(MAX), [rep].[getdummyvaluebyattributebk](a.bk, '-1', '<Empty>', '1900-01-01') + ' AS ' + quotename(a.attributename)) , ',') within GROUP (
                                                                                                                                                                                                       ORDER BY cast(a.ordinalposition AS int)) , markerdescription = ''
         ...
         WHERE 1 = 1
           ...
           AND cast(isnull(a.ismta, 0) AS int) = 0 --and src.BK_RefType_ObjectType = 'OT|T|Table'

         GROUP BY src.bk_dataset ,

                  src.code
       )
SELECT bk = concat(mb.bk_dataset, '|', mb.marker, '|', 'Dynamic') ,
       ...
       mb.markervalue --, len_value = len(MB.MarkerValue)
,
