...
CREATE proc [bld].[load_100_dataset_035_customstagingviews] AS /*
	...
	exec [bld].[load_100_Dataset_035_CustomStagingViews]*/  DECLARE @routinename varchar(8000) = 'load_100_Dataset_035_CustomStagingViews' DECLARE @startdatetime datetime2 = getutcdate() DECLARE @enddatetime datetime2 DECLARE @duration bigint -- Create a helper temp table
 IF object_id('tempdb..#100_Dataset_035_CustomStagingViews') IS NOT NULL
DROP TABLE # 100_dataset_035_customstagingviews ; PRINT '-- create temp table:'
SELECT mta_bk = src.[bk] ,
       ...
       mta_rectype = CASE
            ...
            ELSE -99
             ...
             END INTO # 100_dataset_035_customstagingviews

  FROM [bld].[tr_100_dataset_035_customstagingviews] src

  LEFT JOIN [bld].[vw_dataset] tgt
    ON src.[bk] = tgt.[bk]
CREATE clustered INDEX [ix_tr_100_dataset_035_customstagingviews]
    ON # 100_dataset_035_customstagingviews([mta_bkh] ASC, [mta_rh] ASC) --------------------- start loading data
 PRINT '-- new records:'
...
SELECT src.[bk],
  ...
  JOIN # 100_dataset_035_customstagingviews h
    ON h.[mta_bk] = src.[bk]
...
SELECT src.[bk],
  ...
  LEFT JOIN # 100_dataset_035_customstagingviews h
 ...
 WHERE 1 = 1
   ...
   AND src.mta_source = '[bld].[tr_100_Dataset_035_CustomStagingViews]'
 ...
 IF object_id('tempdb..#100_Dataset_035_CustomStagingViews') IS NOT NULL
  DROP TABLE # 100_dataset_035_customstagingviews;
   ...
   SET @duration = datediff(ss, @startdatetime, @enddatetime) PRINT 'Load "load_100_Dataset_035_CustomStagingViews" took ' + cast(@duration AS varchar(10)) + ' second(s)'
